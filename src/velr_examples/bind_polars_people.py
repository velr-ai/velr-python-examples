...
"""Bind a Polars DataFrame into Velr with the Velr Python driver.

This example shows how to take tabular data from Polars, bind it into
...
We then bind the Polars DataFrame as an external table and use:
...
def main() -> None:
    ...
    with Velr.open(None) as db:
        ...
        with db.exec_one(
            ...
            print("people loaded from Polars into the graph:")
        ...
        # Export the graph data back to Polars.
        out = db.to_polars(
            r"""
            MATCH (p:Person)
            RETURN

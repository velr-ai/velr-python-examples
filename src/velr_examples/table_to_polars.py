...
"""Convert an existing Velr result table to Polars.

This example shows how to run a query with `exec_one()`, work with the
returned result table, and convert that table directly to a Polars
DataFrame.
...
materialize the same table as a Polars DataFrame when needed.
"""
...
def main() -> None:
    with Velr.open(None) as db:
        ...
        with db.exec_one(
            ...
            # Convert the existing result table directly to Polars.
            df = table.to_polars()

            print()
            print("result table converted to Polars:")
            print(df)
